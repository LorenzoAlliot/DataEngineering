...
def load_data_stg(conn_string, dbschema):
    ...
    load_data_into_db(df_league, 'stg_league', conn)
    load_data_into_db(df_team, 'stg_teams', conn)
    load_data_into_db(df_venue, 'stg_venues', conn)
    load_data_into_db(df_country, 'stg_countrys', conn)
    load_data_into_db(df_standings, 'final_standings', conn)



def copy_data(source_table, target_table, conn):
    try:
        conn.begin()
        
        # Elimina registros en la tabla objetivo que coincidan con la tabla de origen
        ...
        # Inserta los registros de la tabla de origen en la tabla objetivo
        conn.execute(sa.text(f"""
        ...
        conn.commit()
        print(f"Datos copiados de {source_table} a {target_table} exitosamente.")
    except Exception as e:
        conn.rollback()
        print(f"Error al copiar datos de {source_table} a {target_table}: {str(e)}")

def load_data_final(conn_string, dbschema):
     conn = sa.create_engine(conn_string, connect_args={'options': f'-csearch_path={dbschema}'})
     copy_data('stg_leagues','final_league',conn)
     copy_data('stg_teams','final_teams',conn)
     copy_data('stg_veneus','final_veneus',conn)
     copy_data('stg_countrys','final_countrys',conn)